...
export type TopLevelScreenId = 'auth' | 'char-create' | 'game-world';
export type SubViewId = 'dungeon' | 'idle' | 'tower' | 'character' | 'companion' | 'inventory' | 'gacha';
export type ScreenId = TopLevelScreenId | SubViewId;
...
export class ScreenManager {
  ...
  private currentScreen: ScreenId = 'auth';
  private registeredScreens: Map<ScreenId, ScreenLifecycle> = new Map();
  private dungeonAfkTimer: any = null;
  ...
  public registerScreen(id: ScreenId, screen: ScreenLifecycle): void {
    this.registeredScreens.set(id, screen);
  }

  public resetDungeonAfkTimer(): void {
    ...
    if (this.currentScreen === 'dungeon') {
      ...
      this.dungeonAfkTimer = setTimeout(() => {
        if (this.currentScreen === 'dungeon') {
          const dScreen = this.registeredScreens.get('dungeon') as any;
          if (dScreen && dScreen.isAutoBattle) return;
  ...
  public showScreen(screenId: ScreenId): void {
    ...
    if (screenId === 'auth') {
      ...
      const views: SubViewId[] = ['dungeon', 'idle', 'tower', 'character', 'companion', 'inventory', 'gacha'];
      views.forEach(v => {
      ...
      if (screenId === 'dungeon') {
        ...
        if (autoBtnContainer) autoBtnContainer.classList.remove('hidden');
        this.resetDungeonAfkTimer();
      } else {
        if (gameContainer) gameContainer.style.display = 'none';
        if (autoBtnContainer) autoBtnContainer.classList.add('hidden');
        if (placeBg) {
      ...
      ['dungeon-world-tier-container', 'hero-aura-meter-container', 'pet-squad-meter-container', 'soul-killmeter-container', 'dungeon-autobattle-container'].forEach(id => {
        const el = document.getElementById(id);
        if (el) {
          if (isDungeon) el.classList.remove('hidden');
