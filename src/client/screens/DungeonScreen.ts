...
import { ScreenLifecycle, ScreenManager } from './ScreenManager';
import { GameStateService, ElementType, PetAttackType, RarityType, InventoryItem } from '../services/GameStateService';
import { AudioService } from '../services/AudioService';
import { UIService } from '../services/UIService';

export class DungeonScreen implements ScreenLifecycle {
  ...
  public init(): void {
    ...
    class MainGameScene extends Phaser.Scene {
      ...
      private skeletonMinions: any;
      private cursors: any;
      private wasd: any;
      private bgGrid: any;
      private bgCastleSprite: any;
      private spawnTimer: any = null;
      private monsterAttackTimer: any = null;
      private monsterAbilityTimer: any = null;
      private autoBattleTimer: any = null;
      private autoSkillTimer: any = null;
      private locatorGraphics: any;
      private rangeGraphics: any;
      private pickupRangeGraphics: any;
      private auraGraphics: any;
      private enemyAuraGraphics: any;
      private enemyCubeGraphics: any;
      private skillGraphics: any;
      private heroOverheadText: any;
      private isDead: boolean = false;
      private earthRotationAngleX: number = 0;
      ...
      private lastUniquePowerTime: number = 0;
      private isDashing: boolean = false;
      private isHeroMoving: boolean = false;

      // Skill State variables
      private spinningStonesAngle: number = 0;

      // Pet & Cutscene variables
      private petSprite: any = null;
      private petKillCount: number = 0;
      private petState: 'hunting' | 'returning' = 'hunting';
      ...
      createProceduralTextures() {
        ...
        // Handsome Anime Young Murim Swordmaster Textures (Male & Female)
        const gWarriorM = this.make.graphics({ x: 0, y: 0, add: false });
        
        // 1. Flowing Long Hair (Back Layer)
        gWarriorM.fillStyle(0x0f172a, 1); gWarriorM.fillCircle(24, 18, 14);
        
        // 2. Humanoid Body & Layered Silk Murim Robe with Gold Belt
        gWarriorM.fillStyle(0x064e3b, 1); gWarriorM.fillRect(12, 18, 24, 24);
        gWarriorM.fillStyle(0x10b981, 1); gWarriorM.fillRect(16, 18, 16, 24);
        gWarriorM.fillStyle(0xfbbf24, 1); gWarriorM.fillRect(14, 28, 20, 4);
        
        // 3. Handsome Anime Face & Long Hair Bangs
        gWarriorM.fillStyle(0xffd1b3, 1); gWarriorM.fillCircle(24, 13, 10);
        gWarriorM.fillStyle(0x0f172a, 1); gWarriorM.fillCircle(24, 8, 11);
        gWarriorM.fillStyle(0x0f172a, 1); gWarriorM.fillTriangle(14, 10, 20, 16, 18, 8);
        gWarriorM.fillStyle(0x0f172a, 1); gWarriorM.fillTriangle(34, 10, 28, 16, 30, 8);
        
        // 4. Sharp Anime Eyes & Forehead Ribbon
        gWarriorM.fillStyle(0x047857, 1); gWarriorM.fillCircle(20, 13, 2.5); gWarriorM.fillCircle(28, 13, 2.5);
        gWarriorM.fillStyle(0xffffff, 1); gWarriorM.fillCircle(19, 12, 1); gWarriorM.fillCircle(27, 12, 1);
        gWarriorM.fillStyle(0xfbbf24, 1); gWarriorM.fillRect(14, 6, 20, 3);
        
        // 5. Waist Scabbard Sheath where Flying Sword rests
        gWarriorM.fillStyle(0x451a03, 1); gWarriorM.fillRect(32, 22, 12, 5);
        gWarriorM.fillStyle(0xfbbf24, 1); gWarriorM.fillRect(30, 21, 3, 7);
        
        gWarriorM.generateTexture('chibi_warrior_m', 48, 48);

        const gWarriorF = this.make.graphics({ x: 0, y: 0, add: false });
        // Female Murim Swordmaster (Flowing Hair & Pink Silk Robe)
        gWarriorF.fillStyle(0x4c0519, 1); gWarriorF.fillCircle(24, 18, 14);
        gWarriorF.fillStyle(0x831843, 1); gWarriorF.fillRect(12, 18, 24, 24);
        gWarriorF.fillStyle(0xf472b6, 1); gWarriorF.fillRect(16, 18, 16, 24);
        gWarriorF.fillStyle(0xffd1b3, 1); gWarriorF.fillCircle(24, 13, 10);
        gWarriorF.fillStyle(0x4c0519, 1); gWarriorF.fillCircle(24, 8, 11);
        gWarriorF.fillStyle(0xbe185d, 1); gWarriorF.fillCircle(20, 13, 2.5); gWarriorF.fillCircle(28, 13, 2.5);
        gWarriorF.fillStyle(0xffffff, 1); gWarriorF.fillCircle(19, 12, 1); gWarriorF.fillCircle(27, 12, 1);
        gWarriorF.fillStyle(0xf472b6, 1); gWarriorF.fillCircle(14, 6, 5);
        gWarriorF.fillStyle(0x451a03, 1); gWarriorF.fillRect(32, 22, 12, 5);
        ...
        gFlyingSword.lineStyle(6, 0x06b6d4, 0.9); gFlyingSword.lineBetween(8, 24, 40, 24);
        gFlyingSword.lineStyle(3, 0xffffff, 1); gFlyingSword.lineBetween(8, 24, 40, 24);
        gFlyingSword.fillStyle(0xffffff, 1); gFlyingSword.fillTriangle(40, 20, 48, 24, 40, 28);
        gFlyingSword.fillStyle(0xfbbf24, 1); gFlyingSword.fillRect(6, 18, 5, 12);
        gFlyingSword.fillStyle(0x78350f, 1); gFlyingSword.fillRect(0, 22, 7, 4);
        gFlyingSword.generateTexture('flying_sword_sprite', 48, 48);

        const gMageM = this.make.graphics({ x: 0, y: 0, add: false });
        gMageM.fillStyle(0x2563eb, 1); gMageM.fillRect(14, 18, 20, 22);
        gMageM.fillStyle(0xffd1b3, 1); gMageM.fillCircle(24, 14, 11);
        gMageM.fillStyle(0x1d4ed8, 1); gMageM.fillCircle(20, 13, 2.5); gMageM.fillCircle(28, 13, 2.5);
        gMageM.fillStyle(0xffffff, 1); gMageM.fillCircle(19, 12, 1); gMageM.fillCircle(27, 12, 1);
        gMageM.fillStyle(0xf1f5f9, 1); gMageM.fillTriangle(17, 18, 31, 18, 24, 30);
        gMageM.fillStyle(0x1e40af, 1); gMageM.fillTriangle(8, 8, 40, 8, 24, -5);
        gMageM.lineStyle(3, 0x78350f, 1); gMageM.lineBetween(36, 38, 36, 2);
        gMageM.fillStyle(0x38bdf8, 1); gMageM.fillCircle(36, 2, 7);
        gMageM.generateTexture('chibi_mage_m', 48, 48);

        const gMageF = this.make.graphics({ x: 0, y: 0, add: false });
        gMageF.fillStyle(0x2563eb, 1); gMageF.fillRect(14, 18, 20, 22);
        gMageF.fillStyle(0x1e1b4b, 1); gMageF.fillCircle(24, 16, 14);
        gMageF.fillStyle(0xffd1b3, 1); gMageF.fillCircle(24, 14, 10);
        gMageF.fillStyle(0x38bdf8, 1); gMageF.fillCircle(20, 13, 2.5); gMageF.fillCircle(28, 13, 2.5);
        gMageF.fillStyle(0xffffff, 1); gMageF.fillCircle(19, 12, 1); gMageF.fillCircle(27, 12, 1);
        gMageF.fillStyle(0x1e40af, 1); gMageF.fillTriangle(8, 8, 40, 8, 24, -5);
        gMageF.lineStyle(3, 0x78350f, 1); gMageF.lineBetween(36, 38, 36, 2);
        gMageF.fillStyle(0x38bdf8, 1); gMageF.fillCircle(36, 2, 7);
        gMageF.generateTexture('chibi_mage_f', 48, 48);

        const gArcherM = this.make.graphics({ x: 0, y: 0, add: false });
        gArcherM.fillStyle(0x059669, 1); gArcherM.fillRect(14, 18, 20, 22);
        gArcherM.fillStyle(0xffd1b3, 1); gArcherM.fillCircle(24, 14, 11);
        gArcherM.fillStyle(0x064e3b, 1); gArcherM.fillCircle(20, 13, 2.5); gArcherM.fillCircle(28, 13, 2.5);
        gArcherM.fillStyle(0xffffff, 1); gArcherM.fillCircle(19, 12, 1); gArcherM.fillCircle(27, 12, 1);
        gArcherM.fillStyle(0x451a03, 1); gArcherM.fillRect(19, 17, 10, 3);
        gArcherM.fillStyle(0x047857, 1); gArcherM.fillTriangle(10, 10, 38, 10, 24, 1);
        gArcherM.fillStyle(0xef4444, 1); gArcherM.fillTriangle(32, 4, 40, 0, 34, 8);
        gArcherM.lineStyle(3, 0xd97706, 1); gArcherM.strokeTriangle(34, 10, 46, 24, 34, 38);
        gArcherM.generateTexture('chibi_archer_m', 48, 48);

        const gArcherF = this.make.graphics({ x: 0, y: 0, add: false });
        gArcherF.fillStyle(0x059669, 1); gArcherF.fillRect(14, 18, 20, 22);
        gArcherF.fillStyle(0xb45309, 1); gArcherF.fillCircle(24, 16, 14);
        gArcherF.fillStyle(0xffd1b3, 1); gArcherF.fillCircle(24, 14, 10);
        gArcherF.fillStyle(0x059669, 1); gArcherF.fillCircle(20, 13, 2.5); gArcherF.fillCircle(28, 13, 2.5);
        gArcherF.fillStyle(0xffffff, 1); gArcherF.fillCircle(19, 12, 1); gArcherF.fillCircle(27, 12, 1);
        gArcherF.fillStyle(0x047857, 1); gArcherF.fillTriangle(10, 10, 38, 10, 24, 1);
        gArcherF.lineStyle(3, 0xd97706, 1); gArcherF.strokeTriangle(34, 10, 46, 24, 34, 38);
        ...
        gSamuraiM.fillStyle(0x991b1b, 1); gSamuraiM.fillRect(14, 18, 20, 22);
        gSamuraiM.fillStyle(0xffd1b3, 1); gSamuraiM.fillCircle(24, 14, 11);
        gSamuraiM.fillStyle(0x7f1d1d, 1); gSamuraiM.fillCircle(20, 13, 2.5); gSamuraiM.fillCircle(28, 13, 2.5);
        gSamuraiM.fillStyle(0xffffff, 1); gSamuraiM.fillCircle(19, 12, 1); gSamuraiM.fillCircle(27, 12, 1);
        // Samurai Golden Kabuto Helmet & Horns
        gSamuraiM.fillStyle(0x18181b, 1); gSamuraiM.fillRect(12, 2, 24, 8);
        gSamuraiM.fillStyle(0xfbbf24, 1); gSamuraiM.fillTriangle(10, 2, 24, -6, 38, 2);
        // Katana Blade
        gSamuraiM.lineStyle(4, 0xf8fafc, 1); gSamuraiM.lineBetween(32, 26, 48, 4);
        gSamuraiM.lineStyle(2, 0xf59e0b, 1); gSamuraiM.lineBetween(32, 26, 48, 4);
        gSamuraiM.generateTexture('chibi_samurai_m', 48, 48);

        const gSamuraiF = this.make.graphics({ x: 0, y: 0, add: false });
        gSamuraiF.fillStyle(0x831843, 1); gSamuraiF.fillRect(14, 18, 20, 22);
        gSamuraiF.fillStyle(0x18181b, 1); gSamuraiF.fillCircle(24, 16, 14);
        gSamuraiF.fillStyle(0xffd1b3, 1); gSamuraiF.fillCircle(24, 14, 10);
        gSamuraiF.fillStyle(0xbe185d, 1); gSamuraiF.fillCircle(20, 13, 2.5); gSamuraiF.fillCircle(28, 13, 2.5);
        gSamuraiF.fillStyle(0xffffff, 1); gSamuraiF.fillCircle(19, 12, 1); gSamuraiF.fillCircle(27, 12, 1);
        // Cherry Blossom Hair Ornament & Dual Katana
        gSamuraiF.fillStyle(0xf472b6, 1); gSamuraiF.fillCircle(14, 6, 5);
        gSamuraiF.lineStyle(3, 0xf8fafc, 1); gSamuraiF.lineBetween(32, 24, 46, 6);
        ...
        gKatanaBeam.lineStyle(8, 0xf59e0b, 0.9); gKatanaBeam.lineBetween(0, 10, 48, 10);
        gKatanaBeam.lineStyle(4, 0xffffff, 1); gKatanaBeam.lineBetween(0, 10, 48, 10);
        gKatanaBeam.generateTexture('proj_katana_beam', 52, 20);

        // 10 UNIQUE BEAST & CELESTIAL COMPANION GRAPHICS TEXTURES
        const drawPetTexture = (key: string, drawFn: (g: Phaser.GameObjects.Graphics) => void) => {
        ...
        drawPetTexture('pet_flame_drake', g => {
          g.fillStyle(0xd97706, 1); g.fillCircle(24, 26, 16);
          g.fillStyle(0xef4444, 1); g.fillTriangle(6, 20, 18, 14, 12, 34); g.fillTriangle(42, 20, 30, 14, 36, 34);
          g.fillStyle(0xfbbf24, 1); g.fillTriangle(18, 14, 22, 4, 24, 14); g.fillTriangle(30, 14, 26, 4, 24, 14);
          g.fillStyle(0xfef08a, 1); g.fillCircle(18, 22, 3.5); g.fillCircle(30, 22, 3.5);
          g.fillStyle(0x000000, 1); g.fillCircle(18, 22, 1.5); g.fillCircle(30, 22, 1.5);
        });

        // 2. Thunder Kitsune ⚡
        drawPetTexture('pet_thunder_kitsune', g => {
          g.fillStyle(0x0284c7, 1); g.fillCircle(24, 26, 15);
          g.fillStyle(0x38bdf8, 1); g.fillTriangle(10, 20, 18, 6, 22, 22); g.fillTriangle(38, 20, 30, 6, 26, 22);
          g.fillStyle(0xe0f2fe, 1); g.fillTriangle(12, 18, 18, 10, 20, 20); g.fillTriangle(36, 18, 30, 10, 28, 20);
          g.fillStyle(0x38bdf8, 1); g.fillCircle(10, 34, 6); g.fillCircle(38, 34, 6); g.fillCircle(24, 40, 7);
          g.fillStyle(0xffffff, 1); g.fillCircle(18, 24, 3); g.fillCircle(30, 24, 3);
        });

        // 3. Void Behemoth 🌌
        drawPetTexture('pet_void_behemoth', g => {
          g.fillStyle(0x581c87, 0.9); g.fillCircle(24, 24, 18);
          g.fillStyle(0x7e22ce, 1); g.fillCircle(24, 24, 12);
          g.fillStyle(0xc084fc, 1); g.fillCircle(24, 24, 7);
          g.fillStyle(0xffffff, 1); g.fillCircle(24, 24, 3);
          g.lineStyle(3, 0xa855f7, 0.9); g.strokeCircle(24, 24, 21);
        });

        // 4. Ice Fenrir 🐺
        drawPetTexture('pet_ice_fenrir', g => {
          g.fillStyle(0x1e3a8a, 1); g.fillCircle(24, 26, 16);
          g.fillStyle(0x60a5fa, 1); g.fillTriangle(8, 18, 16, 4, 22, 22); g.fillTriangle(40, 18, 32, 4, 26, 22);
          g.fillStyle(0x93c5fd, 1); g.fillTriangle(24, 8, 20, 2, 28, 2);
          g.fillStyle(0x38bdf8, 1); g.fillCircle(18, 24, 3.5); g.fillCircle(30, 24, 3.5);
        });

        // 5. Golden Gryphon 🦅
        drawPetTexture('pet_golden_gryphon', g => {
          g.fillStyle(0xd97706, 1); g.fillCircle(24, 26, 16);
          g.fillStyle(0xfbbf24, 1); g.fillTriangle(4, 20, 18, 10, 14, 36); g.fillTriangle(44, 20, 30, 10, 34, 36);
          g.fillStyle(0xfef08a, 1); g.fillTriangle(20, 24, 28, 24, 24, 36);
          g.fillStyle(0xffffff, 1); g.fillCircle(18, 20, 3.5); g.fillCircle(30, 20, 3.5);
        });

        // 6. Abyssal Kraken 🐙
        drawPetTexture('pet_abyssal_kraken', g => {
          g.fillStyle(0x0f766e, 1); g.fillCircle(24, 20, 16);
          g.fillStyle(0x14b8a6, 1);
          g.fillCircle(10, 36, 5); g.fillCircle(18, 38, 5); g.fillCircle(24, 40, 5); g.fillCircle(30, 38, 5); g.fillCircle(38, 36, 5);
          g.fillStyle(0xccfbf1, 1); g.fillCircle(18, 18, 4); g.fillCircle(30, 18, 4);
          g.fillStyle(0x0f172a, 1); g.fillCircle(18, 18, 2); g.fillCircle(30, 18, 2);
        });

        // 7. Mecha Sentinel 🤖
        drawPetTexture('pet_mecha_sentinel', g => {
          g.fillStyle(0x334155, 1); g.fillRect(12, 12, 24, 24);
          g.lineStyle(3, 0x06b6d4, 1); g.strokeCircle(24, 24, 18);
          g.fillStyle(0x06b6d4, 1); g.fillRect(16, 20, 16, 6);
          g.fillStyle(0xffffff, 1); g.fillRect(18, 21, 12, 4);
        });

        // 8. Star Unicorn 🦄
        drawPetTexture('pet_star_unicorn', g => {
          g.fillStyle(0xf472b6, 1); g.fillCircle(24, 26, 15);
          g.fillStyle(0xfbbf24, 1); g.fillTriangle(20, 14, 28, 14, 24, -2);
          g.fillStyle(0xbe185d, 1); g.fillTriangle(10, 16, 16, 6, 20, 20); g.fillTriangle(38, 16, 32, 6, 28, 20);
          g.fillStyle(0xffffff, 1); g.fillCircle(18, 24, 3.5); g.fillCircle(30, 24, 3.5);
        });

        // 9. Sunfire Lion 🦁
        drawPetTexture('pet_sunfire_lion', g => {
          g.fillStyle(0xf59e0b, 1); g.fillCircle(24, 24, 20);
          g.fillStyle(0x78350f, 1); g.fillCircle(24, 24, 14);
          g.fillStyle(0xfbbf24, 1); g.fillCircle(24, 24, 10);
          g.fillStyle(0x000000, 1); g.fillCircle(19, 21, 2.5); g.fillCircle(29, 21, 2.5);
        });

        // 10. Emerald Serpent 🐉
        drawPetTexture('pet_emerald_serpent', g => {
          g.fillStyle(0x047857, 1); g.fillCircle(24, 24, 17);
          g.fillStyle(0x34d399, 1); g.fillCircle(24, 24, 12);
          g.fillStyle(0xa7f3d0, 1); g.fillTriangle(14, 8, 20, 8, 16, 2); g.fillTriangle(34, 8, 28, 8, 32, 2);
          g.fillStyle(0xffffff, 1); g.fillCircle(18, 22, 3); g.fillCircle(30, 22, 3);
        });

        // 10 Monster Textures
        const generateMonster = (key: string, baseColor: number, accentColor: number, isBoss: boolean = false) => {
          const size = isBoss ? 72 : 48;
          const g = this.make.graphics({ x: 0, y: 0, add: false });
          g.fillStyle(baseColor, 1); g.fillCircle(size / 2, size / 2, (size / 2) - 4);
          g.fillStyle(accentColor, 1); g.fillCircle(size / 2 - 8, size / 2 - 4, 5); g.fillCircle(size / 2 + 8, size / 2 - 4, 5);
          if (isBoss) { g.fillStyle(0xfbbf24, 1); g.fillTriangle(size / 2 - 14, 10, size / 2 + 14, 10, size / 2, 0); }
        ...
        gBossDemon.fillStyle(0x7f1d1d, 1); gBossDemon.fillCircle(36, 36, 30);
        gBossDemon.fillStyle(0xef4444, 1); gBossDemon.fillCircle(36, 36, 22);
        gBossDemon.fillStyle(0xfbbf24, 1); gBossDemon.fillCircle(24, 28, 6); gBossDemon.fillCircle(48, 28, 6);
        gBossDemon.fillStyle(0x000000, 1); gBossDemon.fillCircle(24, 28, 3); gBossDemon.fillCircle(48, 28, 3);
        gBossDemon.fillStyle(0xd97706, 1); gBossDemon.fillTriangle(14, 24, 22, 10, 10, 6); gBossDemon.fillTriangle(58, 24, 50, 10, 62, 6);
        gBossDemon.fillStyle(0xffffff, 1); gBossDemon.fillTriangle(26, 44, 30, 44, 28, 52); gBossDemon.fillTriangle(42, 44, 46, 44, 44, 52);
        gBossDemon.generateTexture('boss_infernal_demon', 72, 72);

        const gBossBehemoth = this.make.graphics({ x: 0, y: 0, add: false });
        gBossBehemoth.fillStyle(0x4c1d95, 1); gBossBehemoth.fillCircle(36, 36, 32);
        gBossBehemoth.fillStyle(0x8b5cf6, 1); gBossBehemoth.fillCircle(36, 36, 24);
        gBossBehemoth.fillStyle(0x06b6d4, 1); gBossBehemoth.fillCircle(24, 28, 7); gBossBehemoth.fillCircle(48, 28, 7);
        gBossBehemoth.fillStyle(0xffffff, 1); gBossBehemoth.fillCircle(24, 28, 3); gBossBehemoth.fillCircle(48, 28, 3);
        gBossBehemoth.fillStyle(0x06b6d4, 1); gBossBehemoth.fillTriangle(36, 6, 28, 18, 44, 18); gBossBehemoth.fillTriangle(16, 14, 12, 28, 26, 22); gBossBehemoth.fillTriangle(56, 14, 60, 28, 46, 22);
        ...
        gSkely.fillStyle(0xf8fafc, 1); gSkely.fillCircle(18, 16, 14);
        gSkely.fillStyle(0x0f172a, 1); gSkely.fillCircle(12, 14, 3.5); gSkely.fillCircle(24, 14, 3.5);
        ...
        gRune.fillStyle(0xfbbf24, 1); gRune.fillCircle(16, 16, 12);
        gRune.fillStyle(0xec4899, 1); gRune.fillCircle(16, 16, 8);
        gRune.lineStyle(2, 0xffffff, 1); gRune.strokeCircle(16, 16, 14);
        gRune.generateTexture('drop_rune', 32, 32);

        const gKey = this.make.graphics({ x: 0, y: 0, add: false });
        gKey.fillStyle(0xfbbf24, 1); gKey.fillCircle(16, 10, 8);
        gKey.lineStyle(4, 0xfbbf24, 1); gKey.lineBetween(16, 18, 16, 28); gKey.lineBetween(16, 24, 22, 24);
        gKey.generateTexture('drop_key', 32, 32);

        // Projectiles
        const gSlash = this.make.graphics({ x: 0, y: 0, add: false });
        gSlash.lineStyle(6, 0x10b981, 1); gSlash.arc(18, 18, 16, -0.8, 0.8, false);
        gSlash.generateTexture('proj_slash', 36, 36);

        const gOrb = this.make.graphics({ x: 0, y: 0, add: false });
        gOrb.fillStyle(0xef4444, 1); gOrb.fillCircle(12, 12, 10);
        gOrb.lineStyle(3, 0xfbbf24, 1); gOrb.strokeCircle(12, 12, 12);
        ...
        gArrow.lineStyle(10, 0x34d399, 0.4); gArrow.lineBetween(0, 18, 56, 18); // Trailing Aura
        gArrow.lineStyle(6, 0x10b981, 1); gArrow.lineBetween(4, 18, 56, 18);   // Main Shaft
        gArrow.lineStyle(3, 0xffffff, 1); gArrow.lineBetween(10, 18, 56, 18);  // White Core Shaft
        // Massive Glowing Arrowhead
        gArrow.fillStyle(0xfbbf24, 1); gArrow.fillTriangle(44, 4, 64, 18, 44, 32);
        gArrow.fillStyle(0xffffff, 1); gArrow.fillTriangle(48, 8, 62, 18, 48, 28);
        // Feather Fletching
        gArrow.fillStyle(0x34d399, 1); gArrow.fillTriangle(0, 4, 12, 18, 0, 18); gArrow.fillTriangle(0, 32, 12, 18, 0, 18);
        ...
        gDragon.fillStyle(0xd97706, 1); gDragon.fillEllipse(32, 34, 46, 24); // Body
        gDragon.fillStyle(0xef4444, 1); gDragon.fillTriangle(44, 20, 60, 16, 52, 32); // Dragon Head
        gDragon.fillStyle(0xfbbf24, 1); gDragon.fillCircle(54, 20, 3); // Eye
        gDragon.fillStyle(0xb91c1c, 1); gDragon.fillTriangle(20, 24, 6, 8, 36, 16); // Left Wing
        gDragon.fillStyle(0xb91c1c, 1); gDragon.fillTriangle(20, 24, 6, 44, 36, 32); // Right Wing
        gDragon.fillStyle(0xef4444, 1); gDragon.fillTriangle(14, 32, 0, 36, 18, 38); // Spiked Tail
        gDragon.fillStyle(0x78350f, 1); gDragon.fillRect(24, 24, 16, 14); // Saddle
        gDragon.lineStyle(2, 0xfbbf24, 1); gDragon.strokeRect(24, 24, 16, 14);
        gDragon.generateTexture('mount_flame_dragon', 64, 64);

        const gStallion = this.make.graphics({ x: 0, y: 0, add: false });
        gStallion.fillStyle(0x1e3a8a, 1); gStallion.fillEllipse(32, 34, 42, 22); // Horse Body
        gStallion.fillStyle(0x3b82f6, 1); gStallion.fillRect(44, 18, 10, 20); // Neck
        gStallion.fillStyle(0x60a5fa, 1); gStallion.fillTriangle(44, 18, 60, 14, 52, 26); // Head
        gStallion.fillStyle(0x38bdf8, 1); gStallion.fillTriangle(34, 10, 48, 12, 40, 22); // Electric Mane
        gStallion.fillStyle(0x0284c7, 1); gStallion.fillRect(16, 42, 5, 14); gStallion.fillRect(38, 42, 5, 14); // Hooves
        gStallion.fillStyle(0x78350f, 1); gStallion.fillRect(24, 24, 16, 14); // Saddle
        gStallion.lineStyle(2, 0x38bdf8, 1); gStallion.strokeRect(24, 24, 16, 14);
        gStallion.generateTexture('mount_thunder_stallion', 64, 64);

        const gPhoenix = this.make.graphics({ x: 0, y: 0, add: false });
        gPhoenix.fillStyle(0xd97706, 1); gPhoenix.fillEllipse(32, 34, 40, 22); // Phoenix Body
        gPhoenix.fillStyle(0xfbbf24, 1); gPhoenix.fillTriangle(42, 22, 58, 18, 48, 30); // Bird Head
        gPhoenix.fillStyle(0xf59e0b, 1); gPhoenix.fillTriangle(22, 26, 4, 6, 36, 18); // Left Feather Wing
        gPhoenix.fillStyle(0xf59e0b, 1); gPhoenix.fillTriangle(22, 26, 4, 46, 36, 34); // Right Feather Wing
        gPhoenix.fillStyle(0xfbbf24, 1); gPhoenix.fillTriangle(16, 32, 0, 22, 0, 42); // Golden Feather Tail
        gPhoenix.fillStyle(0x78350f, 1); gPhoenix.fillRect(24, 24, 16, 14); // Saddle
        gPhoenix.lineStyle(2, 0xfbbf24, 1); gPhoenix.strokeRect(24, 24, 16, 14);
        gPhoenix.generateTexture('mount_celestial_phoenix', 64, 64);

        const gDrake = this.make.graphics({ x: 0, y: 0, add: false });
        gDrake.fillStyle(0x311042, 1); gDrake.fillEllipse(32, 34, 44, 26); // Void Body
        gDrake.fillStyle(0x6b21a8, 1); gDrake.fillTriangle(44, 20, 60, 16, 52, 32); // Drake Head
        gDrake.fillStyle(0xc084fc, 1); gDrake.fillCircle(54, 20, 3); // Eye
        gDrake.fillStyle(0x581c87, 1); gDrake.fillTriangle(20, 24, 4, 6, 36, 16); // Left Void Wing
        gDrake.fillStyle(0x581c87, 1); gDrake.fillTriangle(20, 24, 4, 46, 36, 32); // Right Void Wing
        gDrake.fillStyle(0xa855f7, 1); gDrake.fillTriangle(14, 32, 0, 36, 18, 38); // Tail
        gDrake.fillStyle(0x1e1b4b, 1); gDrake.fillRect(24, 24, 16, 14); // Saddle
        gDrake.lineStyle(2, 0xa855f7, 1); gDrake.strokeRect(24, 24, 16, 14);
      ...
      create() {
        ...
        this.input.on('pointerdown', (pointer: any) => {
          ...
          clickedObjects.forEach((obj: any) => {
            if (obj.isEnemy) {
              hitEnemy = true;
      ...
      renderRadarMinimap() {
        ...
        ctx.beginPath(); ctx.arc(center, center, 24, 0, Math.PI * 2); ctx.stroke();
        ctx.beginPath(); ctx.arc(center, center, 48, 0, Math.PI * 2); ctx.stroke();
        ctx.beginPath(); ctx.arc(center, center, 68, 0, Math.PI * 2); ctx.stroke();
      ...
      runAutomaticSkillLogic() {
        ...
        if (skill.skillId === 'spinning_stone') {
          ...
          if (this.skeletonMinions.getChildren().length < 2) {
            ...
            (minion as any).hp = 100;
            this.skeletonMinions.add(minion);
          }

          // Minions attack enemies & drain HP if idle
          this.skeletonMinions.getChildren().forEach((m: any) => {
            if (!m.active) return;
            let nearEnemy: any = null;
            let minDist = Infinity;

            this.enemies.getChildren().forEach((e: any) => {
              if (!e.active) return;
              const d = Phaser.Math.Distance.Between(m.x, m.y, e.x, e.y);
              if (d < minDist) { minDist = d; nearEnemy = e; }
            });
      ...
      updatePorterCollector() {
        ...
        if (this.porterGraphics && this.porterText) {
          ...
          if (porter.rarity === 'rare') ringColor = 0x38bdf8;
          if (porter.rarity === 'legendary') ringColor = 0xf59e0b;
          if (porter.rarity === 'mythic') ringColor = 0xa855f7;
      ...
      ensureMinimumMonsters(minCount: number = 10) {
        ...
        const hasMegaBoss = activeEnemies.some((e: any) => e.isMegaBoss);
        ...
        for (let i = 0; i < needed; i++) {
          ...
          const monsterVariants = [
            { key: 'm_slime_emerald', name: 'Emerald Slime', lvl: 1, hp: 40, element: 'nature' as ElementType, isBoss: false },
            { key: 'm_slime_ruby', name: 'Ruby Slime', lvl: 3, hp: 55, element: 'fire' as ElementType, isBoss: false },
            { key: 'm_goblin', name: 'Goblin Scout', lvl: 5, hp: 65, element: 'none' as ElementType, isBoss: false },
            { key: 'm_drake', name: 'Shadow Drake', lvl: 8, hp: 85, element: 'fire' as ElementType, isBoss: false },
            { key: 'm_skeleton', name: 'Skeleton Warrior', lvl: 12, hp: 110, element: 'none' as ElementType, isBoss: false },
            { key: 'm_beholder', name: 'Void Beholder', lvl: 15, hp: 140, element: 'lightning' as ElementType, isBoss: false },
            { key: 'm_demon', name: 'Frost Demon', lvl: 18, hp: 180, element: 'lightning' as ElementType, isBoss: false },
            { key: 'm_golem', name: 'Infernal Golem', lvl: 22, hp: 230, element: 'fire' as ElementType, isBoss: false },
            { key: 'm_wyvern', name: 'Dark Wyvern', lvl: 25, hp: 300, element: 'nature' as ElementType, isBoss: false },
            { key: 'boss_infernal_demon', name: '👹 Infernal Archdemon', lvl: 30, hp: 600, element: 'fire' as ElementType, isBoss: true },
            { key: 'boss_abyssal_behemoth', name: '👾 Abyssal Behemoth', lvl: 35, hp: 800, element: 'lightning' as ElementType, isBoss: true }
          ];
      ...
      autoSpawnLoop() {
        if (ScreenManager.getInstance().getCurrentScreen() !== 'dungeon' || this.isDead || (this as any).isSpawnPaused || this.isCutsceneActive) return;
        this.ensureMinimumMonsters(10);
      }

      spawnRandomEnemy(x: number, y: number, spriteKey: string, name: string, lvl: number, baseHp: number, element: ElementType, isBoss: boolean = false, isMegaBoss: boolean = false) {
      ...
      spawnElementRuneDrop(x: number, y: number, element: ElementType) {
        const drop = this.add.sprite(x, y, 'drop_rune');
        (drop as any).isKey = false;
        (drop as any).element = element;
      ...
      spawnTowerKeyDrop(x: number, y: number) {
        const drop = this.add.sprite(x, y, 'drop_key');
        (drop as any).isKey = true;
      ...
      performRollDash(moveDx: number, moveDy: number) {
        const now = this.time.now;
        ...
        if (dirX === 0 && dirY === 0) {
          dirX = 1; dirY = 0; // Default right dash if stationary
        }
      ...
      renderSuperSaiyanAura() {
        ...
        if (rune?.element === 'fire') { flameColor = 0xef4444; coreColor = 0xfbbf24; }
        else if (rune?.element === 'lightning') { flameColor = 0x38bdf8; coreColor = 0x60a5fa; }
        else if (rune?.element === 'nature') { flameColor = 0x10b981; coreColor = 0x34d399; }
        ...
        this.menacingTexts.forEach((mText: any, k: number) => {
      ...
      renderAngryEntitiesAndHoppingAnimation() {
        ...
        this.enemies.getChildren().forEach((e: any, index: number) => {
          if (!e.active) return;
      ...
      renderHeroDungeonHUD() {
        ...
        if (iconEl) iconEl.innerText = jobClass === 'WARRIOR' ? '🗡️' : jobClass === 'SAMURAI' ? '🥷' : jobClass === 'MAGE' ? '🔮' : '🏹';
      ...
      updateMountDisplay() {
        ...
        if (this.flyingSwordSprite && !(this as any).isFlyingSwordAttacking) {
      ...
      attackTowardsPointer(targetX: number, targetY: number) {
        if (this.isDead) return;
        ...
        // COMMAND TELEKINETIC FLYING SWORD ATTACK
        (this as any).isFlyingSwordAttacking = true;
        ...
        if (this.flyingSwordSprite) {
          ...
          this.tweens.add({
            ...
            ease: 'Power2',
            onComplete: () => {
              // Perform vertical / horizontal slash FX on target
              const isVertical = Math.random() > 0.5;
              const slashGraphic = this.add.graphics().setDepth(30);

              if (isVertical) {
                slashGraphic.lineStyle(8, 0x06b6d4, 0.95);
                slashGraphic.lineBetween(targetX, targetY - 45, targetX, targetY + 45);
                slashGraphic.lineStyle(4, 0xffffff, 1);
                slashGraphic.lineBetween(targetX, targetY - 45, targetX, targetY + 45);
              } else {
                slashGraphic.lineStyle(8, 0xf59e0b, 0.95);
                slashGraphic.lineBetween(targetX - 50, targetY, targetX + 50, targetY);
                slashGraphic.lineStyle(4, 0xffffff, 1);
                slashGraphic.lineBetween(targetX - 50, targetY, targetX + 50, targetY);
              }

              this.tweens.add({ targets: slashGraphic, alpha: 0, scaleX: 1.6, scaleY: 1.6, duration: 200, onComplete: () => slashGraphic.destroy() });

              this.checkRangeImpact(targetX, targetY);

              // Return flying sword back to scabbard sheath beside waist
              this.tweens.add({
                targets: this.flyingSwordSprite,
                x: this.player.x + 18,
                y: this.player.y + 4,
                ...
                ease: 'Power2',
                onComplete: () => {
                  (this as any).isFlyingSwordAttacking = false;
                  if (this.flyingSwordSprite) this.flyingSwordSprite.setRotation(-Math.PI / 4);
                }
              });
            }
          });
      ...
      triggerWaveCompletedNotice(clearedWave: number, nextWave: number, worldTierNum: number) {
        self.audio.playSound('levelup');
      ...
      rewardMythicMountFromBoss(x: number, y: number) {
        const mountPool: InventoryItem[] = [
      ...
      triggerWorldTierIncreasedNotice(newTierNum: number) {
        self.audio.playSound('levelup');
      ...
      updateCompanionPetLogic() {
        ...
        activePets.forEach((petData, index) => {
          ...
          if (isSuper) {
            ...
            if (petData.rarity === 'rare') rarityColor = 0x10b981;
            if (petData.rarity === 'epic') rarityColor = 0x06b6d4;
            if (petData.rarity === 'legendary') rarityColor = 0xf59e0b;
            if (petData.rarity === 'mythic') rarityColor = 0xef4444;
      ...
      showFloatingPetText(text: string, x: number, y: number) {
        const el = document.createElement('div');
        el.className = 'level-up-float-text';
        el.innerText = text;
      ...
      triggerCutscene() {
        ...
        const bgImage = document.getElementById('cutscene-bg-image') as HTMLImageElement;
        ...
        if (subEl) {
          if (cutsceneId === 'shadow_arise') subEl.innerText = '🌑 SHADOW MONARCH ARMY ARISE • NOIR VOID SLASH!';
          else if (cutsceneId === 'getsuga_tensho') subEl.innerText = '⚔️ GETSUGA TENSHO • CRESCENT MOON KATANA BLADE!';
          else subEl.innerText = '💥 I AM ATOMIC • SCREEN SHATTERING ATOMIC EXPLOSION!';
        }
        ...
        const cutsceneInterval = setInterval(() => {
          ...
          if (secondsLeft <= 0) {
            ...
            (this as any).isSpawnPaused = true;
            this.time.delayedCall(1500, () => {
              (this as any).isSpawnPaused = false;
  ...
  public triggerSuperPetMode(): void {
    if (this.phaserScene) {
      this.phaserScene.triggerSuperPetMode();
    }
  }

  public triggerHeroTitanAuraMode(): void {
    if (this.phaserScene) {
      this.phaserScene.triggerHeroTitanAuraMode();
    }
  }

  public triggerSoulCutscene(): void {
    if (this.phaserScene) {
