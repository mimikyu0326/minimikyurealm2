...
export class DungeonScreen implements ScreenLifecycle {
  ...
  private lastMeterAutoTriggerTime: number = 0;
  ...
  public init(): void {
    ...
    class MainGameScene extends Phaser.Scene {
      ...
      createProceduralTextures() {
        ...
        const drawAnimeMurimGirl = (key: string, gownColor: number, accentColor: number) => {
          ...
          g.fillCircle(11, 11, 3.5); g.fillCircle(37, 11, 3.5); // Hair Ties

          // 2. Stylish Modern Cyan Hoodie & Black Pleated Skirt Outfit
          g.fillStyle(gownColor, 1); g.fillRect(14, 18, 20, 22); // Hoodie Base
          g.fillStyle(accentColor, 1); g.fillRect(16, 18, 16, 22); // Hoodie Zipper Line
          g.fillStyle(0x18181b, 1); g.fillRect(14, 30, 20, 10); // Black Pleated Skirt
          g.fillStyle(0xfbbf24, 1); g.fillRect(14, 29, 20, 2);  // Gold Belt Accent

          // 3. Cute Porcelain Soft Anime Face & Hair Bangs
          g.fillStyle(0xffe4e6, 1); g.fillCircle(24, 13, 10); // Face
          g.fillStyle(0xf472b6, 1); g.fillCircle(24, 8, 10.5); // Crown Hair
          g.fillStyle(0xf472b6, 1); g.fillTriangle(15, 10, 20, 16, 18, 7); // Anime Side Bangs
          g.fillStyle(0xf472b6, 1); g.fillTriangle(33, 10, 28, 16, 30, 7);

          // 4. Large Expressive Cyan Eyes & Soft Blush
          g.fillStyle(0xf43f5e, 0.45); g.fillCircle(17, 15, 2.5); g.fillCircle(31, 15, 2.5); // Soft Blush
          g.fillStyle(0x06b6d4, 1); g.fillCircle(19, 13, 3); g.fillCircle(29, 13, 3);         // Cyan Anime Eyes
          g.fillStyle(0xffffff, 1); g.fillCircle(18, 12, 1.2); g.fillCircle(28, 12, 1.2);    // Highlights

          // 5. Yellow Star Hair Clip & Scabbard Sheath
          g.fillStyle(0xfbbf24, 1); g.fillCircle(12, 9, 3); g.fillCircle(36, 9, 3); // Star Clips
          g.fillStyle(0x78350f, 1); g.fillRect(32, 22, 12, 5); // Sheath
          g.fillStyle(0xfbbf24, 1); g.fillRect(30, 21, 3, 7);
        ...
        gFlyingSword.lineStyle(12, 0x06b6d4, 0.45); gFlyingSword.lineBetween(8, 24, 54, 24); // Cyan Outer Aura
        gFlyingSword.lineStyle(8, 0x0284c7, 0.95); gFlyingSword.lineBetween(8, 24, 52, 24); // Deep Blue Blade
        gFlyingSword.lineStyle(4, 0xffffff, 1); gFlyingSword.lineBetween(8, 24, 52, 24);   // White Sharp Core Edge
        gFlyingSword.fillStyle(0xffffff, 1); gFlyingSword.fillTriangle(48, 16, 62, 24, 48, 32); // Blade Tip
        gFlyingSword.fillStyle(0xfbbf24, 1); gFlyingSword.fillRect(8, 16, 6, 16); // Gold Guard Hilt
        gFlyingSword.fillStyle(0x78350f, 1); gFlyingSword.fillRect(0, 21, 9, 6);  // Leather Handle Grip
        gFlyingSword.fillStyle(0xf59e0b, 1); gFlyingSword.fillCircle(0, 24, 4);   // Gold Pommel Jewel
      ...
      runAutoBattleLogic() {
        if (!this.isAutoBattle || ScreenManager.getInstance().getCurrentScreen() !== 'dungeon' || this.isDead) {
          return;
        }
      ...
      updateMountDisplay() {
        ...
        if (!eqMount) {
          ...
          this.mountSprite.setVisible(false);
          if (this.heroOverheadText) this.heroOverheadText.setPosition(centerX, centerY - 65 + idleFloat);
          return;
        }
      ...
      attackTowardsPointer(targetX: number, targetY: number) {
        if (this.isDead || (this as any).isFlyingSwordAttacking) return;
