    ...
    switchBanner(banner) {
        ...
        if (banner === 'gear') {
            ...
            if (multiIcon)
                multiIcon.innerText = '🎁';
            if (btnSingle)
                btnSingle.innerText = '100 🪙 Gold';
            if (btnMulti)
                btnMulti.innerText = '10 💎 Gems';
        }
        else if (banner === 'pet') {
            if (btnGear)
            ...
            if (multiIcon)
                multiIcon.innerText = '🐉';
            if (btnSingle)
                btnSingle.innerText = '100 🔴 Red Gems';
            if (btnMulti)
                btnMulti.innerText = '10 🟣 Purple Gems';
        }
        else {
            if (btnGear)
                btnGear.className = 'w-full p-3.5 rounded-2xl text-left border transition relative overflow-hidden bg-slate-950/40 backdrop-blur-sm border-purple-500/40 hover:bg-purple-950/50 group';
            ...
            if (multiIcon)
                multiIcon.innerText = '📖';
            if (btnSingle)
                btnSingle.innerText = '100 📜 Skill Tomes';
            if (btnMulti)
                btnMulti.innerText = '10 📖 Ancient Books';
        }
    }
    roll(count) {
        if (this.activeBanner === 'pet') {
            ...
            if (isMulti)
                this.gameState.state.purpleGems -= purpleCost;
            else
                this.gameState.state.redGems -= redCost;
        }
            ...
            if (isMulti)
                this.gameState.state.ancientBooks -= bookCost;
            else
                this.gameState.state.skillTomes -= tomeCost;
        }
        else {
            ...
            if (isGems)
                this.gameState.state.gems -= cost;
            else
