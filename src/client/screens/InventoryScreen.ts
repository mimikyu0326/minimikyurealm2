...
import { GameStateService, InventoryItem, RarityType } from '../services/GameStateService';
import { AudioService } from '../services/AudioService';
import { UIService } from '../services/UIService';

export class InventoryScreen implements ScreenLifecycle {
  ...
  public init(): void {
    ...
    (window as any).toggleEquippedSlotLock = (slotType: string) => {
    ...
    (window as any).autoEquipBestPetsFromUI = () => {
  ...
  private getRarityWeight(rarity: RarityType): number {
  ...
  public updateEquippedSidebar(): void {
  ...
  public renderInventory(): void {
    ...
    items.forEach((item, index) => {
      ...
      const isEquipped =
        (this.gameState.state.equippedWeapon?.id === item.id) ||
    ...
    (window as any).toggleSingleItemLock = (itemId: string) => {
  ...
  private updateBatchSellBar(): void {
  ...
  public isItemEquipped(item: InventoryItem): boolean {
  ...
  public showItemDetailModal(item: InventoryItem, index: number): void {
    ...
    if (titleEl) titleEl.innerText = `${item.isLocked ? '🔒 ' : ''}${item.name}`;
    if (rarityEl) {
      rarityEl.innerText = item.rarity.toUpperCase();
      rarityEl.className = `inline-block px-3 py-1 text-xs font-black rounded-lg uppercase tracking-wider mb-4 ${
        item.rarity === 'mythic' ? 'bg-purple-900 text-purple-200' :
  ...
  public sellSingleItem(item: InventoryItem, index: number): void {
    if (item.isLocked) {
  ...
  private unequipItemSilent(item: InventoryItem): void {
    if (this.gameState.state.equippedWeapon?.id === item.id) this.gameState.state.equippedWeapon = null;
    if (this.gameState.state.equippedArmor?.id === item.id) this.gameState.state.equippedArmor = null;
    if (this.gameState.state.equippedRune?.id === item.id) this.gameState.state.equippedRune = null;
    if (this.gameState.state.equippedSkill?.id === item.id) this.gameState.state.equippedSkill = null;
  ...
  public equipItem(item: InventoryItem): void {
  ...
  public unequipItem(item: InventoryItem): void {
    this.unequipItemSilent(item);
  ...
  public autoEquipHighestCP(): void {
    const inv = this.gameState.state.inventory || [];
    if (inv.length === 0) {
    ...
    const jobClass = this.gameState.state.jobClass || 'WARRIOR';
    let bestWeapon: InventoryItem | null = null;
    let bestArmor: InventoryItem | null = null;
    let bestRune: InventoryItem | null = null;
    let bestSkill: InventoryItem | null = null;
    let bestUniquePower: InventoryItem | null = null;
    let bestMount: InventoryItem | null = null;
    ...
    let equippedCount = 0;
    if (bestWeapon && !this.gameState.state.isWeaponLocked) { this.gameState.state.equippedWeapon = bestWeapon; equippedCount++; }
    if (bestArmor && !this.gameState.state.isArmorLocked) { this.gameState.state.equippedArmor = bestArmor; equippedCount++; }
    if (bestRune && !this.gameState.state.isRuneLocked) { this.gameState.state.equippedRune = bestRune; equippedCount++; }
    if (bestSkill && !this.gameState.state.isSkillLocked) { this.gameState.state.equippedSkill = bestSkill; equippedCount++; }
    if (bestUniquePower && !this.gameState.state.isUniquePowerLocked) { this.gameState.state.equippedUniquePower = bestUniquePower; equippedCount++; }
    if (bestMount && !this.gameState.state.isMountLocked) { this.gameState.state.equippedMount = bestMount; equippedCount++; }
  ...
  public usePotion(item?: InventoryItem, index?: number): void {
