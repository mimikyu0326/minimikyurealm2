...
export class TowerScreen implements ScreenLifecycle {
  ...
  public challengeAllKeysAutoTower(): void {
    const keyCount = Math.min(20, this.gameState.state.towerKeys || 0);
    if (keyCount <= 0) {
    ...
    const rewardsMap = new Map<string, { name: string; icon: string; count: number }>();
    let keysUsed = 0;
  ...
  public showAutoTowerSummaryModal(rewards: { name: string; icon: string; count: number }[], keysUsed: number, startFloor: number, endFloor: number): void {
