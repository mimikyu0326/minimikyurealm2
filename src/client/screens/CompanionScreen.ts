...
import { GameStateService, InventoryItem } from '../services/GameStateService';
import { AudioService } from '../services/AudioService';
import { UIService } from '../services/UIService';

export class CompanionScreen implements ScreenLifecycle {
  ...
  public renderCompanionView(): void {
    this.renderEquippedPetCard();
    this.renderCompanionGrid();
  }

  private renderEquippedPetCard(): void {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      if (pet) {
        let atkBadge = '🐾 SLASH';
        if (pet.petAttackType === 'sniper') atkBadge = '🎯 SNIPER';
        if (pet.petAttackType === 'laser') atkBadge = '⚡ LASER';
        if (pet.petAttackType === 'mage') atkBadge = '🔮 MAGE';
        if (pet.petAttackType === 'shield') atkBadge = '🛡️ SHIELD';
    ...
    (window as any).handleUnequipPet = (petId: string) => {
  ...
  private renderCompanionGrid(): void {
  ...
  public showCompanionDetailModal(pet: InventoryItem): void {
    ...
    if (nameEl) nameEl.innerText = `${pet.isLocked ? '🔒 ' : ''}${pet.name}`;
    ...
    if (btnEquip) {
      ...
      btnEquip.onclick = () => {
        if (isEquipped) {
          (window as any).handleUnequipPet(pet.id);
        } else {
