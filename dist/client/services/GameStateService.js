...
exports.GameStateService = exports.PORTER_VARIANTS = void 0;
...
class GameStateService {
    ...
    manualSaveCharacterRecord() {
        ...
        if (saveBtn) {
            ...
            setTimeout(() => {
                if (saveBtn)
                    saveBtn.className = 'px-3 py-1 bg-emerald-950 hover:bg-emerald-800 text-emerald-300 border border-emerald-500/80 font-black rounded-xl text-xs shadow-lg transition flex items-center gap-1 cursor-pointer';
            }, 1200);
        }
    ...
    loadFromSavedCharacter(charData) {
        ...
        if (this.state.redGems === undefined)
            this.state.redGems = 0;
        if (this.state.purpleGems === undefined)
    ...
    startIdleVaultTimer() {
        setInterval(() => {
            ...
            if (idleView && !idleView.classList.contains('hidden')) {
                ...
                if (expEl)
                    expEl.innerText = `+${this.state.idleVault.accumulatedExp.toLocaleString()} EXP`;
                if (goldEl)
                    goldEl.innerText = `+${this.state.idleVault.accumulatedGold.toLocaleString()} 🪙`;
                if (timeEl)
                    timeEl.innerText = `⏱️ Accumulated: ${this.getIdleVaultDurationText()}`;
            }
        }, 2000);
    }
    ...
    recalculateCP() {
        ...
        if (this.state.equippedWeapon)
            cp += (this.state.equippedWeapon.cpBonus || 20) * (this.state.equippedWeapon.level || 1);
        if (this.state.equippedArmor)
            cp += (this.state.equippedArmor.cpBonus || 15) * (this.state.equippedArmor.level || 1);
        if (this.state.equippedRune)
            cp += (this.state.equippedRune.cpBonus || 30) * (this.state.equippedRune.level || 1);
        if (this.state.equippedSkill)
            cp += (this.state.equippedSkill.cpBonus || 40) * (this.state.equippedSkill.level || 1);
        if (this.state.equippedUniquePower)
            cp += (this.state.equippedUniquePower.cpBonus || 1000) * (this.state.equippedUniquePower.level || 1);
        if (this.state.equippedCutscene)
            cp += (this.state.equippedCutscene.cpBonus || 100) * (this.state.equippedCutscene.level || 1);
        if (this.state.equippedMount)
            cp += (this.state.equippedMount.cpBonus || 2500) * (this.state.equippedMount.level || 1);
        if (this.state.equippedPorter)
            cp += (this.state.equippedPorter.cpBonus || 30) * (this.state.equippedPorter.level || 1);
    ...
    updateHUDDOM() {
        ...
        ['char-name', 'sys-char-name'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.innerText = this.state.name;
        });
        ['char-class', 'hud-class', 'sys-char-class'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.innerText = `${this.state.gender || 'MALE'} ${this.state.jobClass}`;
        });
        ['char-lvl', 'hud-level', 'sys-char-lvl'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.innerText = this.state.level.toString();
        });
        ['char-cp', 'sys-char-cp', 'top-cp-banner'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.innerText = `${this.state.cp} CP`;
        });
        const rankEl = document.querySelector('.rank-badge');
        if (rankEl)
            rankEl.textContent = this.getRankTitle();
        const ptsEl = document.getElementById('stat-points');
        if (ptsEl)
            ptsEl.innerText = `${this.state.statPoints} PTS`;
        ...
        if (autoStatBadge) {
            if (this.state.isAutoAllocateStats) {
                ...
                if (autoStatBtn)
                    autoStatBtn.className = 'px-3 py-1 bg-amber-600 text-slate-950 font-black text-[10px] rounded-xl border border-amber-300 shadow-lg cursor-pointer transition flex items-center gap-1 scale-105';
            }
            else {
                autoStatBadge.innerText = 'OFF';
                autoStatBadge.className = 'text-gray-400 font-extrabold';
                if (autoStatBtn)
                    autoStatBtn.className = 'px-3 py-1 bg-slate-800 hover:bg-slate-700 text-amber-300 font-black text-[10px] rounded-xl border border-amber-500/60 shadow cursor-pointer transition flex items-center gap-1';
            }
        }
        ['str', 'int', 'agi', 'vit'].forEach(s => {
            const el = document.getElementById(`val-${s}`);
            if (el)
                el.innerText = this.state[s].toString();
        });
        ...
        ['hud-gold', 'res-gold', 'gacha-hud-gold'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.innerText = this.state.gold.toLocaleString();
        });
        ['hud-gems', 'res-gems', 'gacha-hud-gems'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.innerText = this.state.gems.toLocaleString();
        });
        ['res-red-gems', 'gacha-hud-redgems'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.innerText = (this.state.redGems || 0).toLocaleString();
        });
        ['res-purple-gems', 'gacha-hud-purplegems'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.innerText = (this.state.purpleGems || 0).toLocaleString();
        });
        ['res-skill-tomes', 'gacha-hud-skilltomes'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.innerText = (this.state.skillTomes || 0).toLocaleString();
        });
        ['res-ancient-books', 'gacha-hud-ancientbooks'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.innerText = (this.state.ancientBooks || 0).toLocaleString();
        });
        ['hud-keys', 'res-keys'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.innerText = `${Math.min(20, this.state.towerKeys || 0)}/20`;
        });
        ...
        ['side-left-icon', 'side-right-icon'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.innerText = sideIcon;
        });
        ['side-weapon-left', 'side-weapon-right'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.className = `w-14 h-44 glass-panel ${borderColor} rounded-full flex flex-col items-center justify-around py-4 text-3xl shadow-2xl animate-float`;
        });
        ['side-left-bar', 'side-right-bar'].forEach(id => {
            const el = document.getElementById(id);
            if (el)
                el.className = `w-2 h-20 bg-gradient-to-b ${sideBarGradient} rounded-full`;
        });
    ...
    performAscension() {
        ...
        if (existing) {
            ...
            if (!this.state.equippedCutscene)
                this.state.equippedCutscene = newItem;
            this.notify();
            this.saveToFirebase();
            return {
                success: true,
    ...
    equipPet(pet) {
        if (!this.state.equippedPets)
            this.state.equippedPets = [];
    ...
    autoEquipBestCutscene() {
        if (this.state.isCutsceneLocked)
            return;
    ...
    grantOrUpgradePorter(variantId) {
        let variant;
        if (variantId) {
            variant = exports.PORTER_VARIANTS.find(v => v.id === variantId) || exports.PORTER_VARIANTS[0];
        }
        else {
            const rand = Math.random();
            if (rand < 0.10)
                variant = exports.PORTER_VARIANTS[3]; // Mythic (10%)
            else if (rand < 0.30)
                variant = exports.PORTER_VARIANTS[2]; // Legendary (20%)
            else if (rand < 0.60)
                variant = exports.PORTER_VARIANTS[1]; // Rare (30%)
            else
                variant = exports.PORTER_VARIANTS[0]; // Common (40%)
        }
